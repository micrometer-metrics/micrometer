...
public class PrometheusMeterRegistry extends MeterRegistry {
    ...
    // The Exemplarsampler use for Counter and for Histogram.
    // In practice these two variables will point to same object (is user use
    // ExamplarConfig in its RegistryConfiguration)
    @Nullable
    private final CounterExemplarSampler counterExemplarSample;
    @Nullable
    private final HistogramExemplarSampler histogramExemplarSample;
    ...
    public PrometheusMeterRegistry(PrometheusConfig config, CollectorRegistry registry, Clock clock) {
        this(config, registry, clock, null, null);
    }

    /*
     * Ctor that accept the Examplar Samplers
     */
    public PrometheusMeterRegistry(PrometheusConfig config, CollectorRegistry registry, Clock clock,
            @Nullable CounterExemplarSampler counterExemplarSample,
            @Nullable HistogramExemplarSampler histogramExemplarSample) {
        super(clock);
        ...
        TraceSamplingCompliantExemplarSampler traceSamplingCompliantSampler = new TraceSamplingCompliantExemplarSampler(
                new OtelMicroMeterSpanContextSupplier());
        if (counterExemplarSample == null) {
            this.counterExemplarSample = traceSamplingCompliantSampler;
        } else {
            this.counterExemplarSample = counterExemplarSample;
        }

        if (histogramExemplarSample == null) {
            this.histogramExemplarSample = traceSamplingCompliantSampler;
        } else {
            this.histogramExemplarSample = histogramExemplarSample;
        }
     ...
     * 
     *         Prometheus content type is replace with Open metrics (it doesn't
     *         matter; if user doesn't request for openmetrics it always keep
     *         text/plain;version=0.0.4;charset=utf-8 content type)
     */
    public String scrape() {
        return scrape(TextFormat.CONTENT_TYPE_OPENMETRICS_100);
    }
    ...
    public void scrape(Writer writer) throws IOException {
        scrape(writer, TextFormat.CONTENT_TYPE_OPENMETRICS_100);
    }
     ...
     *
     * @param contentType   the Content-Type of the scrape.
     * @param includedNames Sample names to be included. All samples will be
     *                      included if {@code null}.
     ...
     *
     * @param writer        Target that serves the content to be scraped by
     *                      Prometheus.
     * @param contentType   the Content-Type of the scrape.
     * @param includedNames Sample names to be included. All samples will be
     *                      included if {@code null}.
    ...
    public Counter newCounter(Meter.Id id) {
        PrometheusCounter counter = new PrometheusCounter(id, counterExemplarSample);
        applyToCollector(id, (collector) -> {
            List<String> tagValues = tagValues(id);
            // Here we add exemplar at Counter creation. It will be use at scrape time
    ...
    public DistributionSummary newDistributionSummary(Meter.Id id,
        ...
        // We Store Sampler in our custom Distribution/Histogram
        PrometheusDistributionSummary summary = new PrometheusDistributionSummary(id, clock,
                distributionStatisticConfig, scale, prometheusConfig.histogramFlavor(), histogramExemplarSample);
        applyToCollector(id, (collector) -> {
            List<String> tagValues = tagValues(id);
            collector.add(tagValues, (conventionName, tagKeys) -> {
                ...
                if (histogramCounts.length > 0) {
                    ...
                    case Prometheus:
                        ...
                        for (CountAtBucket c : histogramCounts) {
                            ...
                            // We retrieve examplar for current bucket through Distribution Class
                            samples.add(new Collector.MetricFamilySamples.Sample(sampleName, histogramKeys,
                                    histogramValues, c.count(), summary.getExemplar(c.bucket())));
                        }
                        ...
                        if (Double.isFinite(histogramCounts[histogramCounts.length - 1].bucket())) {
                            ...
                            // For specifig upbound bucket, we consider an infinite value for upper. Allow
                            // to identify
                            // all measures above the last bucket
                            samples.add(new Collector.MetricFamilySamples.Sample(sampleName, histogramKeys,
                                    histogramValues, count, summary.getExemplar(Double.POSITIVE_INFINITY)));
                        }
                        break;
                ...
                // No exemplars for aggregate counter because it doesn't have sense.
                samples.add(
    ...
    private void addDistributionStatisticSamples(DistributionStatisticConfig distributionStatisticConfig,
            MicrometerCollector collector, HistogramSupport histogramSupport, List<String> tagValues,
     ...
     * MeterRegistry.Config#onMeterRegistrationFailed(BiConsumer)} when you want
     * meters with the same name but different tags to cause an unchecked exception.
     *
