...
class PrometheusMeterRegistryIntegrationTest {
    ...
    @Container
    static GenericContainer<?> prometheus = new GenericContainer<>(DockerImageName.parse(getPrometheusImageName()))
    ...
    private static String getPrometheusImageName() {
        String imageName = System.getProperty("prometheus-image.name");
        if (imageName == null) {
            throw new IllegalStateException(
                    "System property 'prometheus-image.name' is not set. This should be set in the build configuration for running from the command line. If you are running PrometheusMeterRegistryIntegrationTest from an IDE, set the system property to the desired prom/prometheus image name.");
        }
        return imageName;
    }
