    ...
    default Validated<?> validate() {
        return checkAll(this,
                ...
                checkRequired("apiVersion", InfluxConfig::apiVersion)
                        .andThen(v -> v.invalidateWhen(a -> a == InfluxApiVersion.V2 && StringUtils.isBlank(this.org()), "requires 'org' is configured", InvalidReason.MISSING))
                        .andThen(v -> v.invalidateWhen(a -> a == InfluxApiVersion.V2 && StringUtils.isBlank(this.token()), "requires 'token' is configured", InvalidReason.MISSING)),
