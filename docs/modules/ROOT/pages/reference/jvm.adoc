...
----
<1> Provides basic meters for executor activity (see below).
<2> Provides the same meters and timers for tasks (see below).
...
NOTE: The `openTelemetryConventions()` convenience method may configure additional conventions in future minor releases as new OpenTelemetry semantic conventions become stable and are supported by Micrometer. If you require strict metric stability across library upgrades, configure individual conventions explicitly instead.
...
** https://opentelemetry.io/docs/specs/semconv/runtime/jvm-metrics/#metric-jvmmemoryused_after_last_gc[`OpenTelemetryJvmMemoryUsedAfterLastGcMeterConvention`]
* `JvmThreadMetrics`:
