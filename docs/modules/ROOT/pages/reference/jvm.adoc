...
[source, java]
----
ExecutorService monitoredExecutorService = ExecutorServiceMetrics.monitor(registry, executorService, executorServiceName, tags) <1>
// or
new ExecutorServiceMetrics(executor, executorServiceName, tags).bindTo(registry); <2>
----
<1> Provides gauges for executor activity, timers for tasks are also included.
<2> Provides gauges for executor activity.
