...
[source, java]
----
new ExecutorServiceMetrics(executor, executorServiceName, tags).bindTo(registry); <1>
// or
Executor monitoredExecutor = ExecutorServiceMetrics.monitor(registry, executor, executorServiceName, tags) <2>
----
<1> Provides gauges for executor activity, including active threads, idle threads, and queued tasks.
<2> Provides the same gauges and also records timers related to task execution.
...
* `executor` (`Timer`): Tracks the duration of task execution
...
* `executor.idle` (`Timer`): Tracks the duration tasks spend in the queue before execution.
