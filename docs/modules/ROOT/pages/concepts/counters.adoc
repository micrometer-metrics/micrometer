...
To support using the `@MeterTag` annotation, you need to configure the `@CountedAspect` to add the `CountedMeterTagAnnotationHandler`.
...
When the `@MeterTag` annotation is placed on a method parameter, it enables you to add one or more `Tag` to the `Counter` created by `@Counted` using the given method parameter.

Let's assume we have an interface with the following methods:

[source,java,subs=+attributes]
-----
include::{include-java}/metrics/CountedAspectTest.java[tags=interface_for_method_param,indent=0]
-----
...
include::{include-java}/metrics/CountedAspectTest.java[tags=example_param_value_to_string,indent=0]
...
When the `@MeterTag` annotation is placed on a method, it enables you to add one or more `Tag` to the `Counter` created by `@Counted` using the returned value/object of the given method.

Let's assume we have an interface with the following method:

[source,java,subs=+attributes]
-----
include::{include-java}/metrics/CountedAspectTest.java[tags=interface_for_method_result,indent=0]
-----

When its implementations would be called (remember that the implementation needs to be annotated with `@Counted` annotation too), the following counter would be created:

[source,java,subs=+attributes]
-----
// Example for returning <toString()> on the returned value/object
include::{include-java}/metrics/CountedAspectTest.java[tags=example_return_value_to_string,indent=0]
-----
