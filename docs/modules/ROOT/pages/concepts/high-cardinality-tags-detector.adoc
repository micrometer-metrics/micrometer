...
For continuous monitoring in production, you don't need to create a scheduled job that periodically calls the detector. The detector has a built in scheduler, but you don't need to manage that one ether (by calling `start()` and `close()`). Instead you can configure the detector through the `MeterRegistry` so that the detector is automatically started and managed by the registry for you. See the various configuration options with the registry above.
...
If a tag provides little value but high cardinality, remove it:
...
If you are using Micrometer's Observation API, you can mark certain metadata as high cardinality. These key-values are not used for recording metrics but typically only used in outputs that can handle high cardinality (for example: distributed tracing systems, logs):
...
You can use `MeterFilter` to prevent high cardinality before it becomes a problem:
...
5. **Combine with MeterFilters**: Use `maximumAllowableTags` and `maximumAllowableMetrics` as preventive measures alongside the detector.

6. **Test Your Instrumentation**: Use the detector in integration tests to verify that your custom instrumentation doesn't introduce high cardinality.
