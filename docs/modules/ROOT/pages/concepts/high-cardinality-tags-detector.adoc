...
TIP: For more information on tag naming best practices, see xref:./naming.adoc#_tag_naming[Tag Naming].
...
By default, the detector selects the first Meter name whose count exceeds the threshold. You can use `highCardinalityMeterInfoSelector` to select a different candidate, such as the one with the highest count:

[source,java,subs=+attributes]
----
include::{include-java}/metrics/HighCardinalityTagsDetectorTests.java[tags=custom_selector,indent=0]
----

The selector receives a fresh sequential stream containing only candidates above the threshold. It must consume that stream during the call and return an `Optional` containing the selected candidate, or an empty `Optional` to report none. The same selection applies to one-time checks and scheduled monitoring.

Selecting the maximum examines all candidates, while the default selection stops at the first match. Both strategies still count all registered Meters before selecting a candidate.
...
See xref:./meter-filters.adoc[Meter Filters] for more information and configuration options.
...
See xref:./meter-filters.adoc[Meter Filters] for more information.
...
* The detector reports one Meter name per check. By default, this is the first candidate above the threshold; a custom selector can prioritize another candidate. If multiple Meters have high cardinality, you'll need to fix them iteratively.
* The threshold is a heuristic based on memory and may need adjustment for your specific use case.

== See Also

- xref:./meter-filters.adoc[Meter Filters]: Using filters to control meter registration and tag cardinality
