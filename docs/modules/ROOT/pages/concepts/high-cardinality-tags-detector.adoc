...
NOTE: `age` and `status` are not necessarily high cardinality data since they usually fall into a finite set of values (except if input is not validated and `age=12345` and `status=54321` are possible to send to the app). The normalization techniques to reduce cardinality are still valid though.
