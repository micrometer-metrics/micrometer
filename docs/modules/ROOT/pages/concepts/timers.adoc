...
=== The `@MeterTag` Annotation

The `@MeterTag` annotation can be used on:

* Method parameters (to add information about the given parameter)
* Methods (to add information about the returned value/object of the given method)

To support using the `@MeterTag` annotation, you need to configure the `@TimedAspect` to add the `MeterTagAnnotationHandler`.
...
When the `@MeterTag` annotation is placed on a method parameter, it enables you to add one or more `Tag` to the `Timer` created by `@Timed` using the given method parameter.

Let's assume we have an interface with the following methods:

[source,java,subs=+attributes]
-----
include::{include-java}/metrics/TimedAspectTest.java[tags=interface_for_method_param,indent=0]
-----
...
include::{include-java}/metrics/TimedAspectTest.java[tags=example_param_value_to_string,indent=0]
...
WARNING: Please make sure that the attached `Tag` values are sanitized/normalized so that they are low-cardinality values.

==== `@MeterTag` on Methods

When the `@MeterTag` annotation is placed on a method, it enables you to add one or more `Tag` to the `Timer` created by `@Timed` using the returned value/object of the given method.

Let's assume we have an interface with the following method:

[source,java,subs=+attributes]
-----
include::{include-java}/metrics/TimedAspectTest.java[tags=interface_for_method_result,indent=0]
-----

When its implementations would be called (remember that the implementation needs to be annotated with `@Timed` annotation too), the following timer would be created:

[source,java,subs=+attributes]
-----
// Example for returning <toString()> on the returned value/object
include::{include-java}/metrics/TimedAspectTest.java[tags=example_return_value_to_string,indent=0]
-----

The previous examples (using a resolver, defining an expression, or using multiple annotations) also work when `@MeterTag` is placed on a method.
...
NOTE: You need to add LatencyUtils(`org.latencyutils:LatencyUtils`) to your runtime classpath to use the `ClockDriftPauseDetector`.
