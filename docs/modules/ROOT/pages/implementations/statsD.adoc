...
If you intend to use the Datadog or Telegraf flavors, see the documentation for Micrometer's xref:./datadog.adoc[Datadog] or xref:./influx.adoc[Influx] support.

NOTE: StatsD flavors differ in *dimensionality*: Datadog StatsD, Telegraf StatsD, and Sysdig StatsD are *dimensional* (support tags), while Etsy StatsD is *hierarchical*. All flavors use *client-side* xref:../concepts/rate-aggregation.adoc[rate aggregation] and a *server-poll* model (the StatsD agent receives UDP datagrams from the application). See xref:../concepts/implementations.adoc[Supported Monitoring Systems] for a comparison across registries.
...
StatsdConfig config = new StatsdConfig() {
    ...
    @Override
    public String host() {
        return "/var/run/datadog/dsd.socket";
    }
};
