...
NOTE: Prometheus is *dimensional*, uses *server-side* xref:../concepts/rate-aggregation.adoc[rate aggregation] (Micrometer publishes cumulative values; Prometheus computes rates in queries), and operates on a *server-poll* model where Prometheus scrapes the application. See xref:../concepts/implementations.adoc[Supported Monitoring Systems] for a comparison across registries.
...
Enabling xref:../concepts/histogram-quantiles.adoc[percentile histograms] adds a `$\{name}_seconds_bucket` time series; the count, sum, and max series are unchanged.
...
| `$\{name}_seconds_count`, `$\{name}_seconds_sum`,  `$\{name}_seconds_max`
| `$\{name}_seconds_gcount`, `$\{name}_seconds_gsum`, `$\{name}_seconds_bucket`, `$\{name}_seconds_max`
|===

The time series with xref:../concepts/histogram-quantiles.adoc[percentile histograms] enabled are defined by the https://github.com/prometheus/OpenMetrics/blob/main/specification/OpenMetrics.md#gaugehistogram[GaugeHistogram] in the OpenMetrics specification.
...
== Warning on same name with different set of tag keys

Prometheus https://prometheus.io/docs/instrumenting/writing_clientlibs/#labels:[strongly discourages] users from creating meters having the same name with a different set of tag keys. You should register meters with the same name with a consistent set of tag keys.
