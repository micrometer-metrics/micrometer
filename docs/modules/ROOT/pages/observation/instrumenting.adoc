...
Let's look at the following code of HTTP client side instrumentation that reuses the handler.
...
Let's look at an example of the aforementioned handler.
...
Let's look at the code that is the `ProducerInterceptor` for Kafka.
...
Let's look at the following code of the producer side instrumentation that reuses the handler.
...
As an example for the consumer side we will use a handler that instruments the Observation by adding the `foo` low cardinality key with the value being the matched path from the message. Let's look at an example of the `KafkaReceiverContext`.
...
Let's look at the code that is the `ConsumerInterceptor` for Kafka.
...
Let's look at the following code of the consumer side instrumentation that reuses the handler.
