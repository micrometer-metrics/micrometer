...
/**
 * This class contains all deprecated metrics from Micrometer 1.0.x.
 * We keep them here probably until Micrometer 2.x to ease upgrade.
 *
 * @author Clint Checketts
 */
@NonNullApi
@NonNullFields
public class MicrometerMetricsPublisherCommandDeprecated10x implements HystrixMetricsPublisherCommand {
    /*
    ...
    */
    ...
    private static final String DESCRIPTION_HYSTRIX_COMMAND_OTHER = "DEPRECATED since Micrometer 1.1.x. Other execution results. See https://github.com/Netflix/Hystrix/wiki/Metrics-and-Monitoring#other-command-event-types-comnetflixhystrixhystrixeventtype for type definitions";
    /*
    ...
    */
    private static final String DESCRIPTION_HYSTRIX_FALLBACK = "DEPRECATED since Micrometer 1.1.x. Fallback execution results. See https://github.com/Netflix/Hystrix/wiki/Metrics-and-Monitoring#command-fallback-event-types-comnetflixhystrixhystrixeventtype for type definitions";
    ...
    */
    ...
    public MicrometerMetricsPublisherCommandDeprecated10x(MeterRegistry meterRegistry, HystrixCommandKey commandKey, HystrixCommandGroupKey commandGroupKey, HystrixCommandMetrics metrics, HystrixCircuitBreaker circuitBreaker, HystrixCommandProperties properties, HystrixMetricsPublisherCommand metricsPublisherForCommand) {
        ...
        */
        // this will be filled by the new metrics with an additional "terminal" tag.
    ...
    public void initialize() {
        ...
        /* this is already published by the non-deprecated class
        ...
        */
        ...
        fallbackEvents.forEach(hystrixEventType -> {
            eventCounters.put(hystrixEventType, getFallbackCounter(hystrixEventType));
        });
        Arrays.stream(HystrixEventType.values()).filter(e -> !executionEvents.contains(e) && !fallbackEvents.contains(e))
            .forEach(hystrixEventType -> {
                eventCounters.put(hystrixEventType, getOtherExecutionCounter(hystrixEventType));
            });
                ...
                */
                for (HystrixEventType hystrixEventType : HystrixEventType.values()) {
                    ...
                    if (count > 0) {
                        ...
                        Counter counter = eventCounters.get(hystrixEventType);
                        if (counter != null) {
