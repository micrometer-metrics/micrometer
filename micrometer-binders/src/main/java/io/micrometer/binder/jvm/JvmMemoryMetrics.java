...
public class JvmMemoryMetrics implements MeterBinder {
    ...
    public void bindTo(MeterRegistry registry) {
        ...
        for (MemoryPoolMXBean memoryPoolBean : ManagementFactory.getPlatformMXBeans(MemoryPoolMXBean.class)) {
            ...
            Gauge.builder("jvm.memory.used", memoryPoolBean, (mem) -> JvmMemory.getUsageValue(mem, MemoryUsage::getUsed))
                .tags(tagsWithId)
                .description("The amount of used memory")
                .baseUnit(BaseUnits.BYTES)
                .register(registry);

            Gauge.builder("jvm.memory.committed", memoryPoolBean, (mem) -> JvmMemory.getUsageValue(mem, MemoryUsage::getCommitted))
                .tags(tagsWithId)
                .description("The amount of memory in bytes that is committed for the Java virtual machine to use")
                .baseUnit(BaseUnits.BYTES)
                .register(registry);

            Gauge.builder("jvm.memory.max", memoryPoolBean, (mem) -> JvmMemory.getUsageValue(mem, MemoryUsage::getMax))
                .tags(tagsWithId)
