 ...
 * <p>
 * To create a new instance of this class, invoke {@link TestObservationRegistryAssert#assertThat(TestObservationRegistry)}
...
public class TestObservationRegistryAssert extends ObservationRegistryAssert<TestObservationRegistryAssert, TestObservationRegistry> {
    ...
    private void failForNoObservations() {
        failWithMessage("There are no observations registered. You have forgotten to start your observation. Remember to call <observation.start()> method");
    }
    ...
    private String observationNames(List<TestObservationRegistry.TestObservationContext> contexts) {
        return contexts.stream()
                .map(m -> m.getContext().getName()).collect(Collectors.joining(","));
    }
