...
public class JvmSafepointMetrics implements MeterBinder, Closeable {
    ...
    static final String JFR_EVENT_SAFEPOINT_BEGIN = "jdk.SafepointBegin";
    static final String JFR_EVENT_SAFEPOINT_END = "jdk.SafepointEnd";
    static final String JFR_EVENT_SAFEPOINT_STATE_SYNCHRONIZATION = "jdk.SafepointStateSynchronization";
    ...
    private void handleExecuteVmOperation(RecordedEvent event, MeterRegistry registry) {
        // Only VM operations executed at a safepoint are tracked; thread-local handshakes
        // are excluded.
    ...
    private void handleSafepointEnd(RecordedEvent event, MeterRegistry registry) {
        final long safepointId = event.getLong("safepointId");
        final Instant beginTime = safepointBeginCache.remove(safepointId);
        if (beginTime != null) {
            final Duration duration = Duration.between(beginTime, event.getEndTime());
